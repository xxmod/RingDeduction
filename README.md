...
程序会自动检测截图的宽高比（16:9 / 16:10 / 4:3），但在某些情况下需要手动指定：

**原生 16:10（1920×1200、2560×1600 等）**

- 截图像素本身是 16:10 比例
- 程序会自动检测，无需手动配置

**拉伸 16:10（16:10 显示器拉伸模式）**

- 截图像素仍为 16:9（如 1920×1080），但游戏 UI 按 16:10 布局显示
- 需在 `.env` 中手动指定 `ASPECT_RATIO=16:10`
- 程序会自动识别拉伸模式并进行正方化校正
...
- `auto`（默认）：根据截图像素自动判断
- `16:9`：标准 16:9 分辨率（1920×1080、2560×1440 等）
- `16:10`：原生 16:10 分辨率，或 16:10 显示器拉伸模式
- `4:3`：4:3 分辨率（1024×768、1280×960 等，参数待补充）

> **提示：** 程序会自动检测截图实际像素比例与配置是否一致。若不一致（如实际 16:9 但配置 16:10），会自动判定为拉伸模式并进行校正。
...
- 截图需为 **全屏地图界面**（按 M 键打开的大地图）
- 支持多种宽高比：16:9（1920×1080、2560×1440）、16:10（1920×1200、2560×1600）
- 16:10 显示器开启拉伸模式时，请在 `.env` 中设置 `ASPECT_RATIO=16:10`
